...
export function IncidentCard({ incident, onRemove }: { incident: Incident; onRemove?: (id: string) => void }) {
  ...
  const handleVerificationClick = (e: React.MouseEvent, verificationType: string) => {
    e.stopPropagation();
    e.preventDefault();
  ...
  return (
    <Link href={`/incidents/${incident.id}`} className="block">
      <Card className="flex flex-col bg-card hover:bg-muted/50 transition-colors h-full">
        <CardHeader>
          <div className="flex items-start justify-between gap-2">
            <CardTitle className="font-headline text-lg">{incident.type}</CardTitle>
            <Badge
              variant={severityVariantMap[incident.severity] ?? "secondary"}
              className="shrink-0"
            >
              {incident.severity}
            </Badge>
        ...
        <CardFooter className="flex flex-col items-start gap-4">
          ...
          <div className="w-full space-y-2 pt-4 border-t">
            ...
            <div className="grid grid-cols-3 gap-2">
              <Button variant="outline" size="sm" onClick={(e) => handleVerificationClick(e, 'Confirmed')}>
                <Check className="mr-1.5 h-4 w-4" /> Confirm
              </Button>
              <Button variant="outline" size="sm" onClick={(e) => handleVerificationClick(e, 'Unsure')}>
                <HelpCircle className="mr-1.5 h-4 w-4" /> Unsure
              </Button>
              <Button variant="outline" size="sm" className="hover:bg-destructive/10 hover:text-destructive" onClick={(e) => handleVerificationClick(e, 'False')}>
